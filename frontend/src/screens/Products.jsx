...
const Products = () => {
  ...
  useEffect(() => {
    const fetchCategoryName = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5555/api/products/categories/${categoryId}`
        );
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5555/api/products?category=${categoryId}`
        );
  ...
  const validate = () => {
    ...
    if (!formData.email) {
      ...
      !/^[A-Za-z0-9._%+-]+@(gmail\.com|hotmail\.com|outlook\.pt)$/.test(
        formData.email
      )
    ) {
  ...
  const handleDeleteClientFromList = async (clientId) => {
    try {
      await axios.delete(`http://localhost:5555/api/clients/${clientId}`);
      setClients(clients.filter((client) => client._id !== clientId));
      console.log(`Cliente ${clientId} apagado com sucesso.`);
    } catch (error) {
      if (error.response) {
        console.error(
          `Erro ${error.response.status} ao apagar o cliente ${clientId}:`,
          error.response.data
        );
    ...
    if (quantity > 0) {
      ...
      setCartItems((prevItems) => {
        ...
        if (existingItem) {
          ...
          return [
            ...prevItems,
            { ...product, quantity, category: categoryName },
          ];
        }
      });
    } else {
    ...
    try {
      ...
      for (const item of cartItems) {
        ...
        const response = await axios.put(
          `http://localhost:5555/api/products/store/${productId}`,
          { stock: updatedStock }
        );
      ...
      setIsInvoiceModalOpen(false);
      setCheckoutMessage(
        `Checkout realizado com sucesso!${choice ? " Factura emitida." : ""}`
      );
      setTimeout(() => {
        setCheckoutMessage("");
  ...
  return (
    <div className="flex">
      ...
      <div className="ml-5 flex-1">
        <h1 className="text-2xl font-bold mb-5 text-center mt-2">
          {categoryName}
        </h1>
      ...
      {isCartModalOpen && (
        <div
          className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
          onClick={handleCartModalContainerClick}
        >
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex-grow">
                        ...
                        <p className="text-sm">
                          Preço:{" "}
                          {item.price ? `€${item.price}` : "Sem valor definido"}
                        </p>
                      </div>
                      <button
                ...
                <div className="mt-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const stockExceeded = cartItems.some(
                        (item) => item.quantity > item.stock
                      );
                      if (stockExceeded) {
                        // Exibe mensagem de erro caso algum item exceda o estoque
                        setCheckoutMessage(
                          "Excede o stock disponível. Verifique a quantidade dos produtos no carrinho."
                        );
                      } else {
      ...
      {isInvoiceModalOpen && (
        <div
          className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
          onClick={handleInvoiceModalContainerClick}
        >
          <div className="bg-white p-5 rounded-lg w-11/12 md:w-1/2 lg:w-1/3 relative border-gray-400 border-2">
            <IoClose
                  ...
                  <ul className="divide-y divide-gray-200">
                    {clients.map((client) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            setInvoiceName(client.name),
                              setInvoiceNIF(client.nif),
                              setInvoicePhone(client.phone);
                          }}
                          className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 focus:outline-none w-[120px] flex items-center justify-center"
                        >
                          Selecionar
                        </button>
                        <button
                          onClick={() => handleDeleteClientFromList(client._id)}
                          className="text-lg text-red-500 hover:text-red-700 focus:outline-none"
                        >
                          <FaRegTrashAlt />
                        </button>
                  ...
                  <form onSubmit={fetchCreateClient}>
                    <div className="flex flex-wrap">
                      ...
                      <div className="w-full md:w-1/2 pr-2 mb-4">
                        ...
                        />
                        {errors.nif && (
                          <div className="text-red-600">{errors.nif}</div>
                        )}
                      </div>
                      <div className="w-full md:w-1/2 pl-2 mb-4">
                    ...
                    <div className="flex justify-end mt-4">
                      <button
                        type="button"
                        className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 focus:outline-none mr-2"
